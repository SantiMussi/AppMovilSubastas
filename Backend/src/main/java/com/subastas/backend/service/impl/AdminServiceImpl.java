...
public class AdminServiceImpl implements AdminService {
        ...
        public CerrarSubastaResponse cerrarSubasta(Integer empleadoId, Integer subastaId) {
                ...
                return CerrarSubastaResponse.builder()
                                .mensaje(registrosCreados.isEmpty()
                                                ...
                                                : "Subasta cerrada correctamente. Ventas registradas: "
                                                                + registrosCreados.size())
                                .subastaId(subastaId)
                                .estado(subasta.getEstado())
        ...
        private List<RegistroDeSubasta> adjudicarGanadoresPendientes(Subasta subasta) {
                return catalogoRepository.findBySubastaIdentificador(subasta.getIdentificador()).stream()
                                .flatMap(catalogo -> itemCatalogoRepository
                                                .findByCatalogoIdentificador(catalogo.getIdentificador()).stream())
        ...
        private java.util.Optional<RegistroDeSubasta> adjudicarGanadorSiTienePuja(ItemCatalogo item, Subasta subasta) {
                java.util.Optional<RegistroDeSubasta> registroExistente = registroDeSubastaRepository
                                ...
                                .filter(registro -> registro.getSubasta() != null
                                                && registro.getSubasta().getIdentificador()
                                                                .equals(subasta.getIdentificador()))
                                .findFirst();
        ...
        public AdjudicarItemResponse adjudicarItem(Integer empleadoId, Integer itemId, AdjudicarItemRequest req) {
                ...
                // Buscar el siguiente ítem no adjudicado del mismo catálogo, en orden de
                // identificador
        ...
        public void adjudicarItemAutomaticamente(Integer itemId) {
                ...
                adjudicarAGanador(item, subasta);
        ...
        public List<AdminUserResponse> obtenerUsuarios() {
                ...
                return usuarioRepository.findAll().stream()
                                .map(usuario -> toAdminUserResponse(usuario,
                                                clientesPorPersonaId.get(getPersonaId(usuario))))
                                .toList();
        }
        ...
        public MessageResponse admitirUsuario(Integer empleadoId, Integer usuarioId) {
                ...
                if (usuario != null) {
                        ...
                        String htmlBody = "<p style=\"font-size:16px;color:#ccc;\">Hola " + nombre + ",</p>" +
                                        ...
                                        +
                                        "<span style=\"font-size:32px;letter-spacing:8px;color:#e5b95c;font-weight:bold;\">"
                                        + code + "</span>" +
                                        "</div>" +
                                        "<p style=\"font-size:14px;color:#888;\">Te esperamos en la próxima subasta exclusiva.</p>";

                        emailService.sendPremiumEmail(usuario.getEmail(), "Registro Aprobado", "Bienvenido a VANTAGE",
                                        htmlBody);
                }
        ...
        private Cliente findClienteByUsuarioOrClienteId(Integer usuarioId) {
                return clienteRepository.findById(usuarioId)
                                .or(() -> usuarioRepository.findById(usuarioId)
                                                ...
                                                .flatMap(persona -> clienteRepository
                                                                .findById(persona.getIdentificador())))
        ...
        @Transactional
        public AsignarPropuestaResponse asignarPropuesta(Integer empleadoId, Integer propuestaId,
                        AsignarPropuestaRequest req) {
                ...
                if (propuesta.getProductoGenerado() == null) {
                        throw new ConflictException(
                                        "La propuesta no tiene un producto generado. El usuario aún no aceptó los términos.");
                }
